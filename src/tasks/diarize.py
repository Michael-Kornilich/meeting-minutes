...
def main() -> None:
    ...
    if Path(f"{config['data-dir']}/meeting-diary.jsonl").exists():
        print("Diary jsonl exists. Skipping the task")
        return

    if Path(f"{config['data-dir']}/diarization.pkl").exists():
        with open(f"{config['data-dir']}/diarization.pkl", mode="rb") as file:
        ...
        with open(f"{config['data-dir']}/diarization.pkl", mode="wb") as file:
    ...
    jsonl.dump(json_lines, f"{config['data-dir']}/meeting-diary.jsonl")
    return
